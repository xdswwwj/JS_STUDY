...
const NEWS_URL: string = "https://api.hnpwa.com/v0/news/@page.json";
...
function newsFeed(): void {
  ...
  for (
    ...
    newsList.push(`
      <div class="p-6 ${
        item.read ? "bg-red-500" : "bg-white"
      } mt-6 rounded-lg shadow-md transition-colors duration-500 hover:bg-green-100">
        <div class="flex">
          <div class="flex-auto"><a href="#/show/${item.id}">${
      item.title
    }</a></div>
          <div class="text-center text-sm">
            <div class="w-10 text-while bg-green-300 rounded-lg px-0 py-2">
              ${item.comments_count}
            </div>
          </div>
        </div>
        <div class="flex mt-3">
          <div class="grid grid-cols-3 text-sm text-gray-500">
            ...
            <div><i class="fas fa-clock mr-1"></i>${item.time_ago}</div>
          </div>
        </div>
      </div>
    `);
  ...
  <div class="bg-gray-600 min-h-screen pb-8">
    <div class="bg-white text-xl">
      <div class="mx-auto px-4">
        <div class="flex justify-between items-center py-6">
          <div class="flex justify-start">
            <div class="h1 font-extrabold">Hacker News</div>
          </div>
          <div class="items-center justify-end">
